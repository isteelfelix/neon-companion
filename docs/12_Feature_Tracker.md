...
| U-54 | Terminal remote exec (Hermes WS RPC) | ⚠️ | M2 | Compatibility bridge exists, but current upstream gateway has no `client.register` / `terminal.execute` / `terminal.respond`; local-agent execution is unavailable until the gateway contract returns |
...
| U-58 | Terminal shell tabs + agent mirrors | 🧪 | M2 | Independent local PTY tabs, close/switch/new chrome, Ctrl/Alt shell key encoding, focus isolation during chat streaming, and read-only `agent.terminal.output` tabs; awaiting Felix Unity/Windows smoke test |
