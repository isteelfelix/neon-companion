...
| A-13 | VRM runtime Phase B | ⏳ | M4 | UniVRM 0.131.2 import/render, actual humanoid/blink/gaze/expression/lipsync capability extraction, restricted fallback, VRMA states, immediate voice-stop clearing, mouse-drag-driven authored VRMC_springBone motion, bust-only idle/turn forcing, and matched body/clothing Bust weights for both main and detached renderers implemented. Felix fixture committed; coat mesh rig remains an asset-side limitation; needs Felix Unity/Windows gallery→preview, spring-bone and TTS/lipsync smoke verification. |
