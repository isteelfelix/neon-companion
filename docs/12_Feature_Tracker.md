...
| A-14 | Windows Companion window Phase C | ⏳ | M4 | Isolated display-only Unity Player, named-pipe profile/state/TTS-text protocol, capability-aware VRM visemes with immediate playback-stop clearing, supervision, compact status/settings UI, Pet hover/context controls, persisted show/pin/click-through/monitor/scale/position controls, transparent/topmost/drag Win32 layer and Ctrl+Shift+F12 emergency release implemented. Needs Felix Windows build evidence: two processes/no second session, screenshots/logs, persisted restart, Pet control smoke, TTS lipsync/clear, child crash/close isolation and parent cleanup. |
