...
| A-13 | VRM runtime Phase B | ⏳ | M4 | UniVRM 0.131.2 import/render, actual humanoid/blink/gaze/expression/lipsync capability extraction, restricted fallback, VRMA states, immediate voice-stop clearing, mouse-drag-driven authored VRMC_springBone motion, and full-amplitude bust-only idle/turn forcing for both main and detached renderers implemented. Covered body triangles are masked beneath Neon's opaque grey top to prevent the painted bra/body mesh from clipping through without changing garment weights. Felix fixture committed; coat mesh rig remains an asset-side limitation; needs Felix Unity/Windows gallery→preview, spring-bone and TTS/lipsync smoke verification. |
| A-14 | Windows Companion window Phase C | ⏳ | M4 | Isolated display-only Unity Player, named-pipe profile/state/TTS-text protocol, capability-aware VRM visemes with immediate playback-stop clearing, supervision, persisted show/pin/click-through/monitor/scale/position controls, transparent/topmost/drag Win32 layer and Ctrl+Shift+F12 emergency release implemented. Needs Felix Windows build evidence: two processes/no second session, screenshots/logs, persisted restart, TTS lipsync/clear, child crash/close isolation and parent cleanup. |
| A-15 | Avatar/Companion hardening Phase D | ⏳ | M4 | Pre-copy mutation and pre-instantiation complexity guards, bounded generic cache, immediate TTS wait cancellation, EditMode regressions, performance observations and ten-item Windows evidence ledger implemented. Linux Phase A–D checks pass; needs Felix Unity EditMode run, licensed VRM 0.x + committed VRM 1.0 evidence, Windows lifecycle harness, UI/TTS smoke and profiler captures. |
...
| U-54 | Terminal remote exec (Hermes WS RPC) | 🧪 | M2 | Client protocol v2 ready: session-scoped local approval, per-chat persistent shells, bounded output/timeouts, safe reconnect reset. Backend must implement `docs/23_Client_Terminal_Protocol.md`; awaiting end-to-end test |
...
| U-58 | Terminal shell tabs + agent mirrors | 🧪 | M2 | Independent local PTY tabs, compact themed tab chrome, Ctrl/Alt shell key encoding, focus isolation during chat streaming, and read-only `agent.terminal.output` tabs; awaiting Felix Unity/Windows smoke test |
...
| V-05 | Проигрывание аудио из бабла | ✅ | M2+ | Verified by Felix (июнь 2026): клик по 🎧 воспроизводит оригинальный WAV (не пересинтез), повторный клик — мгновенный replay. |
| V-06 | Персистентное хранение аудио бабла | 📋 | M2+ | Аудио всё ещё в `temporaryCachePath` с TTL 5 мин (`TempFileMaxAgeSec=300`). `audioPath` сериализуется на `ChatMessage`, но файл удаляется. Нужно: перемещать в `persistentDataPath` для привязанных к сообщению файлов. |
| V-07 | Отложенная отправка аудио на STT | ✅ | M2+ | Verified by Felix (июнь 2026): превью в композере сразу, "Распознаём речь" анимация, текст заполняется после STT. |
