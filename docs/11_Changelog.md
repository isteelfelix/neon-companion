...
### Added
- **Avatar backend import Phase A** — avatar settings now imports static 2D, motion-pack sprite sheets, generic GLB/glTF, and catalog-only VRM through an explicit validation dialog with preview, diagnostics, and evidence-based capabilities. User-owned assets are copied into isolated persistent-data directories only after format/size/scene validation; failed inputs preserve the previous active avatar. `AvatarProfile` now carries a versioned type/source/capability contract and generic-3D state-to-clip mapping while legacy profiles remain readable. VRM rendering, expressions, animation, and lipsync remain Phase B.
