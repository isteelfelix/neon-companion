...
### Fixed
- Neon bust springs now keep their full procedural idle and turn amplitude in both the main preview and detached Companion process. The covered body surface carrying the painted black bra is masked beneath the opaque grey top, preventing body/top intersections without clamping the motion or replacing authored garment weights.
- Neon bust secondary motion now uses a softer, damped, lightly gravity-weighted profile across both authored chains, replacing the rigid high-stiffness response that made the two sides look like tense mechanical springs.
- Neon coat spring metadata now has consistently named `CoatSkirt` groups and a cloth-like parameter profile, but mesh inspection found that the current export lacks usable coat-bone weights. Visible coat motion remains intentionally deferred until the source asset is rigged again.
- VRM secondary motion now reacts when the user horizontally mouse-drags the avatar: the renderer turns the model root instead of orbiting a stationary model, allowing authored `VRMC_springBone` hair, clothing, and body springs to receive motion. Centered springs receive a proportional lateral force, a released flick coasts briefly, and vertical drag retains camera pitch control.
- Companion/avatar acceptance cleanup: the right column now renders exactly one selected backend, stale asynchronous 3D loads cannot reappear over a newly selected sprite, and 3D/VRM no longer inherit the 2D breathing scale. Static 2D, sprite-sheet, generic GLB/glTF and VRM have separate catalog categories; generic glTF import is actually enabled through the supported glTFast API. The detached Windows Pet is forced to start windowed, can be dragged from its visible body, scales with the mouse wheel or hover controls, and exposes compact hover/context actions instead of a permanent control panel. Avatar display scale and X/Y framing are editable and persisted from the avatar settings page.
