...
- The built-in Neon coat is no longer rigid during VRM secondary motion: all 18 authored coat-skirt chains (42 active joints) now use a cloth-like stiffness, damping, and downward-gravity profile. The mislabeled `TopsUpperArm` spring groups are named `CoatSkirt`, and the Phase D fixture check guards the profile against future model re-exports.
