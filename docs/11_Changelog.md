...
### Fixed
- Neon bust springs now receive a bust-only procedural idle force plus turn inertia in both the main preview and detached Companion process. Unconstrained chains receive a conservative cone limit so secondary motion cannot throw the mesh through the clothing.
