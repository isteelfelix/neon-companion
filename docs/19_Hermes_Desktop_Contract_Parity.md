...
| `agent.terminal.output` | stream (keyed by `process_id` only) | YES (`HandleAgentTerminalOutput`) — buffered per owning chat and surfaced in read-only process tabs | P2 |
...
**Companion-only events (remote-client extension, NOT in Desktop union):** `client.ping` (→ `client.pong`), `file.transfer.start` / `.chunk` / `.finish`, and `terminal.execute` (→ `terminal.respond`). Current upstream `tui_gateway` (verified 2026-07-27) registers none of `client.register`, `terminal.execute`, or `terminal.respond`; the agent's ordinary `terminal` tool therefore runs in its gateway/runtime environment, not the Companion host. Companion's client side is ready for the session-scoped v2 bridge, but the gateway must implement the exact contract and separate `client_terminal` tool in `23_Client_Terminal_Protocol.md`. Desktop exposes local user PTYs only to the renderer and gives the agent `read_terminal`; backend background processes are mirrored through `agent.terminal.output`.
