...
| Context limits/UI flags | Provider may store a manual context window; discovery heuristics inspect non-standard fields (`ModelDiscoveryService.cs:51-109`); UI otherwise estimates. `ProviderCapabilities` has no model-level modality/context metadata. | OpenAI model limits and modalities vary by model and change over time. | Medium | Unknown is shown/estimated; no catalog claims added. A model metadata source is required before exposing precise flags. |
