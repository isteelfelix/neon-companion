...
hood/strings, sleeves и hair. Для плаща в файле объявлены 18 цепей и 42 joint'а,
но текущий экспорт меша почти не содержит весов этих костей, поэтому изменение
их stiffness, damping и gravity не может дать видимой деформации. Исправление
плаща требует повторного рига исходного ассета и отложено.
...
Одних мягких коэффициентов недостаточно для неподвижной модели, поэтому runtime
добавляет только четырём `Bust` joint'ам слабую несимметричную idle-нагрузку и
усиленный боковой импульс от вращения аватара. Драйвер работает и в основном
preview, и в отдельном Companion-процессе, не меняя параметры hair/coat springs.
Чтобы сохранить полную амплитуду без клипинга, 356 вершин грудных областей
четырёх слоёв одежды используют не меньший Bust-вес, чем ближайшая поверхность
тела. В этот набор входят ранее неразмеченные нижние участки наружной одежды и
нижний край бюстгальтера. Инструмент переносит веса детерминированно и не ставит
angle limit.
